...
\begin{itemize}
	...
	\setlength{\parskip}{0pt}
	\setlength{\parsep}{0pt}
	\item Grouper used Multipeer Connectivity\cite{mc},  a official peer-to-peer communication framework provided by Apple, to transfer data between devices by a face-to-face way.
	\item Grouper uses Core Data\cite{coredata}, a official ORM framework provided by Apple, to manage the model layer objects. Core Data provides generalized and automated solutions to common tasks associated with object life cycle and object graph management, including persistence.
	\item Grouper uses Sync\cite{sync}, a modern JSON synchronization framework for Core Data, to create JSON strings and synchronize object from JSON strings.
	\item Grouper uses c-SSS\cite{c-sss}, an implementation of Shamir's secret sharing in the C language, to create shares and recover shares.
	\item Grouper uses AFNetworking\cite{afnetworking}, a delightful networking library by Objective-C language, to revoke the RESTful API provided by our Web services. 
\end{itemize}
