...
The Grouper API layer rely on two lower layers: the synchronization layer and the reliable messaging layer.
When an object is updated in a local device, the synchronization layer marshall an updated object and obtains a message.
...
Next, the Grouper API layer updated the object by passing the received message to the synchronization layer.

We have not implemented the synchronization layer by ourselves, but we make this layer as a pluggable module.
...
Applications send update, delete and confirm messages through the Grouper API.
When the applications invokes the method grouper.sender.update() of the Grouper API, the Grouper API layer sends an update message that contains the marshalled object to all devices.
When the applications invokes the method grouper.sender.delete() of the Grouper API, the Grouper API layer sends a delete message that contains the ID of the deleted object to all devices. 
