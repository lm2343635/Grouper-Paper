...
To address the problem of using central servers, the proposals including Vanish\cite{geambasu2009vanish}, SafeVanish\cite{zeng2010safevanish}, SeDas\cite{zeng2012sedas} and CouldSky\cite{zeng2015cloudsky} constructed a data self-destruction system as their cloud storage.
...
Second, these approaches do not provide frameworks for developing mobile applications.
...
When the application needs to synchronize data, it invokes the method $grouper.receiver.$ $receive()$ with a callback function.
...
For this reason, if a developer wants to use another consistency model, the increased LoC of the application will be more than which in Table 4.
...
Figure 6(a) shows the relationship between processing time and the number of servers ${n}$, where ${k=3}$ and ${n = 2, 4, 6, ... , 10}$.
As $n$ increased, the processing time of uploading and downloading increased linearly. 
Figure 6(b) shows the relationship between processing time and the threshold ${k}$, where ${n=30}$ and ${k = 3, 6, 9, ... , 27}$.
...
\begin{acks}
	The authors would like to thank Elvis Nun{\~e}z for helping us to fix some Objective-C related issues of the Sync framework, and Bernard Paulus for fixing a bug of c-SSS, a implementation of the secret sharing scheme.
