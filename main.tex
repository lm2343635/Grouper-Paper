...
\usepackage[a4paper, top=0.5in, bottom=0.7in, left=0.5in, right=0.5in]{geometry}
...
Our research goal is to develop safe light-weight information sharing applications for small groups with our framework Grouper. 
...
They are described in the related work (Section 6).

Inspired by such researches, we proposal our own framework Grouper.
Compared to those proposal, Grouper's contribution is that it ensure both data security and reliability, which is necessary for developing our target applications.
Although Grouper uses the data self-destruction scheme, reliable information sharing can also be ensured.
Data can be recovered even untrusted servers shut down because all devices of group members keep a complete data set of this group.
