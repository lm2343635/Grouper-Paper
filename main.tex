...
%データリカバリ付き自己破壊システム上でのモバイルアプリケーションの開発
\author{Meng Li}
%% name of advisor
...
\majorfield{Computer Science} \degree{Engineering} \yearandmonth{March 2018}
...
%伝統的なモバイルアプリケーションはクライアント-サーバモデルに基いて開発する。こんなモデルは中央サーバでデータを保存し、敏感な情報を処理する。不審者が中央サーバをアクセルすると、クリアテキストで保存した個人的な情報を漏らす可能性がある。本研究は信頼できる中央サーバを利用しないで、モバイルアプリケーションの開発のフレームワークGrouperを実装した。Grouperはデータリカバリ付き自己破壊システム上でのモバイルアプリケーションの開発を支援できる。具体的に言えば、このフレームワークはGrouperメッセージプロトコルを基づいて複数の信頼できないサーバを経由し、複数のモバイルデバイス間オブジェクト同期機能を提供する。GrouperはObjective-C言語で開発したクライアントフレームワークとJava言語で開発したWebサービスを含む。性能評価のために、Grouperで二つのアプリケーションを開発した。アプリケーションの開発は、スタンドアロンアプリケーションからデータ共有アプリケーションを簡単に拡張できることを証明する。性能評価の実験は、Grouperが小規模なグールプのためのアプリケーションの性能を影響しないことを証明する。
\par
...
To address the problem of using central servers, the proposals including Vanish\cite{geambasu2009vanish}, SafeVanish\cite{zeng2010safevanish}, SeDas\cite{zeng2012sedas} and CloudSky\cite{zeng2015cloudsky} constructed a data self-destruction system as their storage.
...
SafeVanish prevents "hopping attacks"\cite{wolchok2010defeating}, which is one kind of the Sybil attacks\cite{cholez2009evaluation, wolchok2010defeating} by extending the length range of key shares to increase the attack cost substantially, and do some improvement on the Shamir's secret sharing scheme\cite{shamir1979share} implemented in the Vanish system.
...
CloudSky deals with the untrusted cloud storage network in two ways including the data self-destruction and the use of Hash-based Message Authentication Code (HMAC) \cite{bellare1997hmac}, which ensures the data integrity.
...
Grouper uses Shamir's secret sharing scheme\cite{shamir1979share} to protect messages from the providers of untrusted servers.
...
We will show the usage of these methods in Section \ref{section:application}.
...
Because we implement our client framework in Objective-C, we currently use the Sync framework\cite{sync} to implement the data synchronization plugin.
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]		
		...
		\State $sendMsg(resendMsg,\ sender)$ \Comment Create a resend message with maximum sequence number.
		\EndIf
		...
		\State $resendMsg \gets createResendMsg(lastMsg.seq\ +\ 1,\ maxSeq)$
		\If{$resendMsg \neq null $}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.62]{receive_confirm}
	\caption{Screenshots of receiving messages and sending a confirm message in the Account Book application.}
	\label{fig:receive_messages_send_confirm}
\end{figure}

\item \textbf{Sending Confirm Messages}

We have the method $grouper.confirm()$ in Section \ref{section:grouper_message_protocol}.
The Account Book application invokes the method in the following two ways.
...
\begin{itemize}[leftmargin=7mm]
	...
	As described int the code from Line 2 to Line 7 of Figure \ref{fig:receive_with_completion}, when the Account Book application receives messages from untrusted servers, it checks the time that the application sent the last confirm message.
...
A device does not download the messages that have been uploaded.
