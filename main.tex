...
\usepackage[a4paper, top=0.5in, bottom=0.6in, left=0.45in, right=0.45in]{geometry}
...
\begin{algorithm}[t]
	...
	\begin{algorithmic}[1]		
		...
		\If{$ msg.seq \neq  0 \ \&\& \ lastMsg.seq + 1 \neq  msg.seq$}
		\State $resendMsg \gets createResendMsg(lastMsg.seq + $ \\\hspace{3.3cm} $1, msg.seq)$
		
		\Comment Create a resend message with the minimum and \\\hspace{1.2cm} maximum sequence number.
		...
		\State $resendMsg \gets createResendMsg(lastMsg.seq + $ \\\hspace{3.2cm}  $1, maxSeq)$
			\If{$resendMsg \neq null $}
			\State $sendMsg(resendMsg, sender)$
			\EndIf
